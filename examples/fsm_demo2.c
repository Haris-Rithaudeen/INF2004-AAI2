...
static bool barcode_detection_enabled = false;
...
// Determine turn direction from barcode character
static int get_turn_direction_from_barcode(char ch) {
    // Move Right if: A, C, E, G, I, K, M, O, Q, S, U, W, Y
    // Move Left if: B, D, F, H, J, L, N, P, R, T, V, X, Z
    
    switch (ch) {
        case 'A': case 'C': case 'E': case 'G':
        case 'I': case 'K': case 'M': case 'O':
        case 'Q': case 'S': case 'U': case 'W':
        case 'Y':
            printf("[BARCODE] '%c' → TURN RIGHT 90°\n", ch);
            return 1;  // Right
        ...
        case 'Z':
            printf("[BARCODE] '%c' → TURN LEFT 90°\n", ch);
            return -1;  // Left
            
        default:
            printf("[BARCODE] ⚠️  Unknown character '%c' - no turn\n", ch);
            return 0;  // No turn
    }
}
...
void fsm_init(void) {
    ...
    // Enable barcode detection by default
...
void fsm_step(void) {
    ...
    // ========== BARCODE DETECTION (ADD THIS SECTION) ==========
    if (barcode_detection_enabled) {
        // Update barcode scanner
        ir_barcode_update();
        
        // Check if a character was detected
        if (ir_barcode_has_char()) {
            char barcode_char = ir_barcode_get_char();
            
            // Only process if it's a new character
            if (barcode_char != last_barcode_char) {
                last_barcode_char = barcode_char;
                
                printf("\n╔═══════════════════════════════════════════════════╗\n");
                printf("║ 🎯 BARCODE DETECTED: '%c'                         ║\n", barcode_char);
                printf("╚═══════════════════════════════════════════════════╝\n\n");
                
                int turn_dir = get_turn_direction_from_barcode(barcode_char);
                ...
                if (turn_dir != 0) {
                    ...
                    // Get current heading
                    if (imu_ready && heading_locked) {
                        magnetometer_data_t mag_data;
                        if (magnetometer_read_data(&mag_data)) {
                            float current_heading = mag_data.heading;
                            float target_heading_local;
                            
                            if (turn_dir < 0) {
                                // Turn LEFT
                                target_heading_local = current_heading - 90.0f;
                                printf("[BARCODE] Executing LEFT 90° turn: %.1f° → %.1f°\n",
                                       current_heading, target_heading_local);
                            } else {
                                // Turn RIGHT
                                target_heading_local = current_heading + 90.0f;
                                printf("[BARCODE] Executing RIGHT 90° turn: %.1f° → %.1f°\n",
                                       current_heading, target_heading_local);
                            }
                            
                            // Normalize target heading
                            while (target_heading_local < 0.0f) target_heading_local += 360.0f;
                            while (target_heading_local >= 360.0f) target_heading_local -= 360.0f;
                            
                            // Execute turn (blocking)
                            uint32_t turn_start = now_ms();
                            bool turn_complete = false;
                            
                            while (!turn_complete && (now_ms() - turn_start) < 8000) {
                                if (magnetometer_read_data(&mag_data)) {
                                    float heading_now = mag_data.heading;
                                    
                                    // Calculate error
                                    float error = target_heading_local - heading_now;
                                    while (error > 180.0f) error -= 360.0f;
                                    while (error < -180.0f) error += 360.0f;
                                    ...
                                    if (abs_error <= 3.0f) {
                                        ...
                                        motor_set_signed(0, 0);
                                        turn_complete = true;
                                        printf("[BARCODE] ✓ Turn complete! Final: %.1f° | Error: %.2f°\n",
                                               heading_now, error);
                                    } else {
                                        // Continue turning
                            ...
                            if (!turn_complete) {
                                printf("[BARCODE] ⚠️  Turn timeout\n");
                                motor_set_signed(0, 0);
                            }
                            
                            // Update target heading for line following
                            target_heading = target_heading_local;
                            
                            sleep_ms(500);
                        }
                    } else {
                        printf("[BARCODE] ⚠️  IMU not ready, skipping turn\n");
                    }
                }
                
                // Reset scanner for next barcode
                ir_barcode_reset();
                ir_barcode_start_scan();
